// ════════════════════════════════════════
//  آمرني — api/generate.js  v7.1
//  Fix: Arabic filename in Content-Disposition
//  Uses RFC 5987 encoding for Unicode filenames
// ════════════════════════════════════════

// Build a safe Content-Disposition header value
// ASCII fallback + RFC 5987 UTF-8 encoded filename*
function contentDisposition(displayName, ext) {
  const fullName  = `${displayName}.${ext}`;
  // ASCII-only fallback (replaces non-ASCII with _)
  const asciiName = `${displayName.replace(/[^\x20-\x7E]/g, '_').replace(/\s+/g,'_')}.${ext}`;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const displayName = (filename || 'document').trim() || 'document';

    switch (type.toLowerCase()) {
      case 'pdf':   return await generatePDF(res, content, displayName, lang);
      case 'docx':  return await generateWord(res, content, displayName, lang);
      case 'xlsx':  return await generateExcel(res, content, displayName, lang);
      case 'pptx':  return await generatePPT(res, content, displayName, lang);
...
async function generatePDF(res, content, filename, lang) {
  ...
  const doc = new PDFDocument({
    ...
    info: { Title: filename, Author: 'AMRNI' }
  });
  ...
  await new Promise(resolve => {
    ...
    // Header bar
    doc.rect(0, 0, pageW, 8).fill('#00bfff');

    // Brand name (ASCII only to avoid font issues)
    doc.fontSize(11).fillColor('#00bfff').font('Helvetica-Bold')
       .text('AMRNI', margin, 22, { align: isAr ? 'right' : 'left' });
    ...
    for (const rawLine of lines) {
      ...
      if (line.startsWith('# ')) {
        ...
        doc.circle(isAr ? pageW - margin - 6 : margin + 6, yPos + 6, 2.5).fill('#00bfff');
        doc.fontSize(10).fillColor('#1a2a3a').font('Helvetica')
           .text(txt,
             isAr ? margin : margin + 16, yPos,
             { width: contentW - 16, align: isAr ? 'right' : 'left' });
        yPos += doc.heightOfString(txt, { width: contentW - 16 }) + 8;
    ...
    for (let i = 0; i < total; i++) {
      ...
      doc.fontSize(8).fillColor('#94b0c0').font('Helvetica')
         .text('AMRNI', margin, pageH - 18, { align: 'center', width: contentW });
    }
  ...
  res.setHeader('Content-Disposition', contentDisposition(filename, 'pdf'));
  ...
  for (const rawLine of content.split('\n')) {
    ...
    if (line.startsWith('# ')) {
      children.push(new Paragraph({
        ...
        children: [new TextRun({ text: line.slice(2), bold: true, size: 36, color: '0a1628', font: isAr ? 'Arial' : 'Calibri' })],
      ...
      children.push(new Paragraph({
        ...
        children: [new TextRun({ text: line.slice(3), bold: true, size: 28, color: '0099cc', font: isAr ? 'Arial' : 'Calibri' })]
      }));
    } else if (line.startsWith('### ')) {
      children.push(new Paragraph({
        heading: HeadingLevel.HEADING_3, alignment: align, bidirectional: isAr,
        spacing: { before: 160, after: 80 },
        children: [new TextRun({ text: line.slice(4), bold: true, size: 24, color: '1a2a3a', font: isAr ? 'Arial' : 'Calibri' })]
      }));
    } else if (line.startsWith('- ') || line.startsWith('* ')) {
      children.push(new Paragraph({
        bullet: { level: 0 }, alignment: align, bidirectional: isAr,
        spacing: { after: 80 },
        children: [new TextRun({ text: line.slice(2), size: 22, font: isAr ? 'Arial' : 'Calibri', color: '1a2a3a' })]
      }));
    } else if (line.startsWith('**') && line.endsWith('**')) {
      children.push(new Paragraph({
        alignment: align, bidirectional: isAr, spacing: { after: 80 },
        children: [new TextRun({ text: line.slice(2, -2), bold: true, size: 22, font: isAr ? 'Arial' : 'Calibri', color: '0a1628' })]
      }));
    } else {
      children.push(new Paragraph({
        alignment: align, bidirectional: isAr, spacing: { after: 100 },
        children: [new TextRun({ text: line, size: 22, font: isAr ? 'Arial' : 'Calibri', color: '1a2a3a' })]
      }));
    }
  }
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [new TextRun({ text: 'AMRNI', size: 18, color: '00bfff', font: isAr ? 'Arial' : 'Calibri' })]
          })]
        })
      },
      footers: {
        default: new Footer({
          children: [new Paragraph({
            ...
            children: [
              new TextRun({ text: isAr ? '\u0635\u0641\u062d\u0629 ' : 'Page ', size: 16, color: '94b0c0', font: isAr ? 'Arial' : 'Calibri' }),
    ...
    if (line.includes('|')) {
      ...
      if (!headerDone) {
        ...
        row.eachCell(cell => {
          ...
          cell.border = { bottom: { style: 'medium', color: { argb: 'FF0099cc' } } };
        });
        headerDone = true;
      } else {
        row.height = 22;
        ...
        row.eachCell(cell => {
          ...
          cell.alignment = { vertical: 'middle', horizontal: isAr ? 'right' : 'left', readingOrder: isAr ? 2 : 1 };
          cell.border = { bottom: { style: 'thin', color: { argb: 'FFe0f5f5' } } };
        });
      }
      rowIndex++;
    } else if (line.startsWith('#')) {
...
async function generatePPT(res, content, filename, lang) {
  ...
  const C = {
    bg: 'FFFFFF', accent: '00bfff', accent2: '0099cc',
    text: '0a1628', sub: '4a6580', light: 'f0fffe',
  };
  ...
  for (const rawLine of content.split('\n')) {
    ...
    if (line.startsWith('# ')) {
      ...
      if (current) current.bullets.push({ text: line.slice(4), bold: true });
    } else if (line.startsWith('- ') || line.startsWith('* ')) {
  ...
  if (current) sections.push(current);
  if (!sections.length) sections.push({ title: filename, subtitle: content.slice(0, 200), bullets: [], type: 'title' });

  sections.forEach((sec, idx) => {
    const slide = prs.addSlide();
    slide.background = { color: C.bg };
    slide.addShape(prs.ShapeType.rect, { x: 0, y: 0, w: '100%', h: 0.12, fill: { color: C.accent } });

    if (sec.type === 'title' && idx === 0) {
      slide.addShape(prs.ShapeType.rect, { x: 3.5, y: 2.2, w: 3, h: 0.06, fill: { color: C.accent }, line: { color: C.accent } });
      slide.addText(sec.title, { x: 0.5, y: 1.2, w: 9, h: 1.4, fontSize: 36, bold: true, color: C.text, fontFace, align: 'c', valign: 'middle', ...rtl });
      if (sec.subtitle) slide.addText(sec.subtitle, { x: 0.5, y: 2.8, w: 9, h: 0.8, fontSize: 18, color: C.sub, fontFace, align: 'c', ...rtl });
      slide.addText('AMRNI', { x: 0.3, y: 6.8, w: 9.4, h: 0.35, fontSize: 10, color: C.accent, align: 'c', fontFace });
    } else {
      slide.addShape(prs.ShapeType.rect, { x: 0, y: 0.12, w: '100%', h: 1.15, fill: { color: C.light }, line: { color: C.light } });
      slide.addText(sec.title, { x: 0.4, y: 0.18, w: 9.2, h: 0.95, fontSize: 24, bold: true, color: C.text, fontFace, align, valign: 'middle', ...rtl });
      slide.addShape(prs.ShapeType.line, { x: 0.4, y: 1.32, w: 9.2, h: 0, line: { color: C.accent, width: 2 } });

      if (sec.subtitle) slide.addText(sec.subtitle, { x: 0.4, y: 1.45, w: 9.2, h: 0.5, fontSize: 13, color: C.sub, fontFace, align, ...rtl });

      if (sec.bullets.length > 0) {
        const startY   = sec.subtitle ? 2.05 : 1.55;
        const bulletObjs = sec.bullets.slice(0, 7).map(b => ({
          text: b.text,
          options: {
            fontSize: 13, color: b.bold ? C.text : '2a3a4a', bold: b.bold,
            bullet: b.bold ? false : { type: 'bullet', code: '25CF', color: C.accent },
            indentLevel: b.bold ? 0 : 1,
            fontFace, ...rtl,
          }
        }));
        slide.addText(bulletObjs, { x: 0.4, y: startY, w: 9.2, h: 6.8 - startY - 0.4, valign: 'top', lineSpacingMultiple: 1.4 });
      }

      slide.addText(`${idx + 1}  |  AMRNI`, { x: 0.3, y: 6.85, w: 9.4, h: 0.28, fontSize: 9, color: C.accent, align: 'c', fontFace });
    }
  });
